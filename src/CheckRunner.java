...
public class CheckRunner {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (args.length == 0 )
        {
            File file = new File("CheckInputFile.txt");
    ...
    public static List<String[]> getGoods(String[] items)
    {
        ...
        for (String item : items) {
            ...
            {
                result.add(temp); // игнорируем строку с картой и ее номер
            }
        }
        return result;
    }
    ...
    public static String getDiscondCardNumber(String[] items)
    {
        for (String item : items) {
            ...
            if (temp[0].equals("card"))
            {
                return temp[1]; // перебираем массив и забираем только карту с номером
            }
        }
        return "";
    }
    ...
    public static void dataScanner(List<String[]> inputData, String cardNumber) throws IOException {
        ...
        for (var item : inputData)
        {
            ...
            {
                lostData.add(String.format("Product with ID #№%s doesn't exist", Integer.toString(searchKey)));
                continue;
            }
        ...
        if (discont == 0)
        {
            lostData.add(String.format("Discount Card №%s doesn't exist", cardNumber));
        }
        lineForPrint = String.format("-".repeat(39) + "\n");
        System.out.print(lineForPrint);
        linesForPrint.add(lineForPrint);
        ...
        if (discont != 0)
        {
            ...
            System.out.printf("TOTAL PRICE WITH DISCOUNT: %-7s \n", String.format("%.2f", totalPrice - totalPrice * discont) + "$");
            linesForPrint.add(String.format("TOTAL PRICE WITH DISCOUNT: %-7s", String.format("%.2f", totalPrice - totalPrice * discont) + "$ \n"));
        }
        for (var data: lostData)
        {
            System.out.println(data);
            linesForPrint.add(data);
        }
